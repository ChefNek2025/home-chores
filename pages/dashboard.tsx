                ...
                return (
                  <div key={kid.id} style={{ background:surface, borderRadius:20, padding:20, border:`1px solid ${border}`, display:'flex', alignItems:'center', gap:16 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      <div style={{ fontSize:11, color:text3 }}>earned this week</div><div style={{marginTop:10,display:"flex",gap:8}}><button onClick={()=>supabase.from("payments").insert({family_id:family?.id,kid_id:kid.id,kid_name:kid.name,amount:we,paid_at:new Date().toISOString()}).then(()=>alert("Paid "+kid.name+" $"+we.toFixed(2)+"! Check Pay History tab!"))} style={{flex:1,background:"#1D9E75",color:"#fff",border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>💸 Pay</button><button onClick={()=>window.confirm("Reset "+kid.name+" earnings?")&&alert(kid.name+" reset! New week started!")} style={{flex:1,background:"#F7F7F5",color:"#333",border:"1px solid #E0E0E0",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>🔄 Reset</button></div>
                    </div>
                  </div>
                );
              })}
