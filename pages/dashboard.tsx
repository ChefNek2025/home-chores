                ...
                return (
                  <div key={kid.id} style={{ background:surface, borderRadius:20, padding:20, border:`1px solid ${border}`, display:'flex', alignItems:'center', gap:16 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      <div style={{ fontSize:11, color:text3 }}>earned this week</div><div style={{marginTop:10,display:"flex",gap:8}}><select onChange={(e)=>{ const m=e.target.value; if(!m) return; const urls={"venmo":"https://venmo.com","zelle":"https://zellepay.com","cashapp":"https://cash.app","step":"https://step.com","current":"https://current.com"}; supabase.from("kids").update({paid_this_week:true,earned_amount:0}).eq("id",kid.id); setKids(kids.map(k=>k.id===kid.id?{...k,earned_amount:0,paid_this_week:true}:k)); setPaidKids(prev=>[...prev,kid.id]); if(urls[m]) window.open(urls[m],"_blank"); else alert("Paid "+kid.name+" $"+we.toFixed(2)+" in cash! ✅"); e.target.value=""; }} style={{flex:1,background:"#1D9E75",color:"#fff",border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>
  <option value="">💸 Pay ${we.toFixed(2)}</option>
  <option value="venmo">💜 Venmo</option>
  <option value="zelle">💜 Zelle</option>
  <option value="cashapp">💚 Cash App</option>
  <option value="step">💙 Step Card</option>
  <option value="current">⚫ Current Card</option>
