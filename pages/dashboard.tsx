                ...
                return (
                  <div key={kid.id} style={{ background:surface, borderRadius:20, padding:20, border:`1px solid ${border}`, display:'flex', alignItems:'center', gap:16 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      <div style={{ fontSize:11, color:text3 }}>earned this week</div><div style={{marginTop:10,display:"flex",gap:8}}><button onClick={()=>{ setKids(prev=>prev.map(k=>k.id===kid.id?{...k,earned_amount:0,paid_this_week:true}:k)); setPaidKids(prev=>[...prev,kid.id]); fetch("/api/pay-kid",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kidId:kid.id,familyId:family?.id,kidName:kid.name,amount:we})}).then(r=>r.json()).then(d=>console.log("paid:",d)); alert("Paid "+kid.name+"! ✅"); }} style={{flex:1,background:"#1D9E75",color:"#fff",border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>💸 Pay ${we.toFixed(2)}</button><button onClick={()=>{ setKids(prev=>prev.map(k=>k.id===kid.id?{...k,earned_amount:0,paid_this_week:false}:k)); setPaidKids(prev=>prev.filter(id=>id!==kid.id)); fetch("/api/reset-kid",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kidId:kid.id})}).then(r=>r.json()).then(d=>console.log("reset:",d)); alert("Reset "+kid.name+"! ✅"); }} style={{flex:1,background:"#F7F7F5",color:"#333",border:"1px solid #E0E0E0",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>🔄 Reset</button></div>
                    </div>
                  </div>
                );
              })}
