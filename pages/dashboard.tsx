                    ...
                    <div key={photo.id} style={{ background:'#fff', borderRadius:20, padding:20, border: '2px solid ' + (photo.status==='approved'?'#1D9E75':photo.status==='rejected'?'#EF3340':'#EBEBEB') }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize:13, color:'#888' }}>{chore?.emoji} {chore?.name || 'Chore'} · {new Date(new Date(photo.created_at).toLocaleString('en-US', {timeZone:'America/New_York'})).toLocaleString('en-US', {month:'short',day:'numeric',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:true})}</div>
                        </div>
