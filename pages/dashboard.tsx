                ...
                {kids.map(kid => (
                  <div key={kid.id} style={{ background:'#fff', borderRadius:20, padding:24, border:'1px solid #EBEBEB' }}>
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      ...
                      <button onClick={() => navigator.clipboard.writeText(window.location.origin + '/kid/' + kid.name.toLowerCase())} style={{ flex:1, background:'#F7F7F5', color:'#333', border:'1px solid #E0E0E0', borderRadius:10, padding:'8px', fontSize:13, fontWeight:600, cursor:'pointer' }}>📋 Copy link</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            {showLibrary && (
              <div style={{ background:'#fff', borderRadius:16, border:'1px solid #EBEBEB', marginBottom:16, maxHeight:280, overflowY:'auto' as any, boxShadow:'0 4px 20px rgba(0,0,0,.08)' }}>
                ...
                {filteredLibrary.length === 0 && <div style={{ padding:20, textAlign:'center', color:'#888', fontSize:14 }}>No matches — add "{choreSearch}" as a custom chore below!</div>}
              </div>
            )}
            <div style={{ background:'#fff', borderRadius:20, padding:24, border:'1px solid #EBEBEB', marginBottom:16 }}>
                    ...
                    <div key={photo.id} style={{ background:'#fff', borderRadius:20, padding:20, border: '2px solid ' + (photo.status==='approved'?'#1D9E75':photo.status==='rejected'?'#EF3340':'#EBEBEB') }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize:13, color:'#888' }}>{chore?.emoji} {chore?.name || 'Chore'} · {(() => { const d = new Date(photo.created_at); d.setHours(d.getHours() - 4); return d.toLocaleString('en-US', {month:'short',day:'numeric',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:true}); })()}</div>
                        </div>
        ...
        {tab === 'paykids' && (
          <div>
            ...
            <div style={{ background:'#fff', borderRadius:20, padding:24, border:'1px solid #EBEBEB' }}>
              <h3 style={{ fontSize:14, fontWeight:700, color:'#0D1117', marginBottom:4 }}>💸 Pay manually</h3>
  ...
  return (
    <div style={{ background:'linear-gradient(135deg,#0D1117,#1a2332)', borderRadius:20, padding:24, border:'1px solid #1D9E75' }}>
      ...
      {report ? (
        ...
        <p style={{ color:'#666', fontSize:14, marginBottom:16, lineHeight:1.6 }}>Generate a personalized AI report about your family's chore progress, earnings, and achievements this week!</p>
      )}
