                ...
                return (
                  <div key={kid.id} style={{ background:surface, borderRadius:20, padding:20, border:`1px solid ${border}`, display:'flex', alignItems:'center', gap:16 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      <div style={{ fontSize:11, color:text3 }}>earned this week</div><div style={{marginTop:10,display:"flex",gap:8}}><button onClick={()=>{ const m=window.prompt("Pay "+kid.name+" via: 1=Venmo 2=Zelle 3=CashApp 4=Step 5=Current 6=Cash 7=Direct"); if(!m) return; supabase.from("kids").update({paid_this_week:true,earned_amount:0}).eq("id",kid.id); setKids(kids.map(k=>k.id===kid.id?{...k,earned_amount:0,paid_this_week:true}:k)); setPaidKids(p=>[...p,kid.id]); const u={"1":"https://venmo.com","2":"https://zellepay.com","3":"https://cash.app","4":"https://step.com","5":"https://current.com"}; if(u[m]) window.open(u[m],"_blank"); else alert("Paid "+kid.name+"! ✅"); }} style={{flex:1,background:"#1D9E75",color:"#fff",border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>💸 Pay ${we.toFixed(2)}</button></div>
                    </div>
                  </div>
                );
              })}
...
function AIReport({ familyId }: { familyId: string }) {
  ...
  return (
    ...
    fetch('/api/pay-kid', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({kidId:kid.id,familyId:family?.id,kidName:kid.name,amount:we})});
    alert("Paid "+kid.name+" $"+we.toFixed(2)+"!");
