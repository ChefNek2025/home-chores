...
const code = `import { GetStaticProps, GetStaticPaths } from 'next';
...
function TodayTab({ kidId, app, toast }: { kidId: KidId; app: ReturnType<typeof useAppState>; toast: ReturnType<typeof useToast> }) {
  ...
  async function uploadPhoto(choreId: string, file: File) {
    try {
      ...
      if (uploadError) { toast.show('Upload failed: ' + uploadError.message); return; }
      const { data: urlData } = supabase.storage.from('chore-photos').getPublicUrl(fileName);
      const { data: { user } } = await supabase.auth.getUser();
      const familyId = user?.id || 'anonymous';
      const { error: dbError } = await supabase.from('chore_photos').insert({
        family_id: familyId,
        kid_id: null,
        chore_id: null,
      ...
      if (dbError) { toast.show('Save failed: ' + dbError.message); return; }
    ...
    <div className="space-y-4">
      ...
      <div className="card space-y-1">
        {chores.map(chore => {
          ...
          return (
            <div key={chore.id} className="space-y-1">
              ...
              {d && (
                <label style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', background:'#F0FBF7', borderRadius:10, cursor:'pointer', fontSize:13, color:'#0F6E56', fontWeight:600 }}>
                  📸 Add photo proof
